...
const MobileAboutMeDetail = memo(({title, description, image}: MobileAboutMeDetailProps) => {
    return (
        <div className="flex flex-col items-center my-8 space-y-4 text-center">
            <h4 className="text-3xl font-[500] pr-[0.6rem] tracking-[.4px] text-black dark:text-white">{title}</h4>
            <p className="mx-4 text-black dark:text-white text-xl pb-[10px] tracking-[0.38px] pt-[0.3rem]">{description}</p>
